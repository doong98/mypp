...
public class BoardController {
    ...
    //@PathVariable("id")는 {id}에서 전달받은 id값을 매개변수로 전달받음.
    ...
    //@RequestParam(value="content") : 사용자가 board_create.html문서에서 작성한 내용을 전달받는 매개변수
    @PostMapping("/create")
    public String create(@Valid BoardForm boardForm, BindingResult bindingResult, Principal principal){
        ...
        //제목과 내용을 boardService에 전달하고 나서 목록페이지로 이동
    ...
    public String modify(BoardForm boardForm, @PathVariable("id") Integer id){
        //boardService의 getOne()메서드를 호출해서 id값에 해당하는 레코드를 board테이블에서 가져와서 board인스턴스에 저장
        Board board = this.boardService.getOne(id);
        //만약 로그인된 아이디와 글 작성한 아이디가 같지 않으면 수정권한 없음
